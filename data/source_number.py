...
def var_delta(delta_lambda):
    ...
    Courtesy of: https://github.com/sreyas-mohan/DeepFreq
    """
    cummean = np.cumsum(delta_lambda[::-1]) / np.arange(
        1, len(delta_lambda) + 1
    )  # mean( \delta_K, ..., \delta_{N-1} )
    delta_lambda_norm = (delta_lambda[None] - cummean[:, None]) ** 2
    var = np.sum(np.triu(delta_lambda_norm), axis=1) / np.arange(
        1, len(delta_lambda) + 1
    )
    return var
...
def AIC(s, param=20):
    ...
    cumprod = np.cumprod(eig_values[::-1])[
        ::-1
    ]  # , 1/np.arange(1, len(eig_values)+1))[::-1]
