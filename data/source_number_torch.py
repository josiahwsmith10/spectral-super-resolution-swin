...
def AIC_torch(s, param=20):
    ...
    cumprod = torch.cumprod(
        eig_values.flip(dims=(0,)),
        dim=0
    ).flip(dims=(0,))  # , 1/torch.arange(1, len(eig_values)+1)).flip(dims=(0,))
    cummean = torch.cumsum(eig_values.flip(dims=(0,)), dim=0) / torch.arange(1, len(eig_values) + 1)
    cummean = torch.pow(cummean, torch.arange(1, len(eig_values) + 1)).flip(dims=(0,))
    log_div = torch.log(cumprod / cummean)
    n_s = torch.arange(1, len(eig_values) + 1)
    m = len(s)
    n = sig.shape[0]
