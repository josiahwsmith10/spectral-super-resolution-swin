...
def chamfer(f_estimate, f_target):
    ...
    Courtesy of: https://github.com/sreyas-mohan/DeepFreq
    """
    f_estimate[f_estimate == -1] = -10.0
    f_target[f_target == -1] = -10.0
    ...
    for i in range(f_target.shape[1]):
        ...
        dist_i_rshift = np.min(
            np.abs((f_estimate + 1) - f_target[:, i][:, None]), axis=1
        )
        dist_i_lshift = np.min(
            np.abs((f_estimate - 1) - f_target[:, i][:, None]), axis=1
        )
        dist_i = np.min((dist_i_direct, dist_i_rshift, dist_i_lshift), axis=0)
        b = f_target[:, i] != -10.0
    ...
    for i in range(f_estimate.shape[1]):
        ...
        dist_i_rshift = np.min(
            np.abs((f_target + 1) - f_estimate[:, i][:, None]), axis=1
        )
        dist_i_lshift = np.min(
            np.abs((f_target - 1) - f_estimate[:, i][:, None]), axis=1
        )
