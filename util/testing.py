...
def test_basic_SR(
    ...
    assert x.dim() == 2 or x.dim() == 3, "Must be 2-D complex-valued or 3-D real-valued with layered I/Q"
    if x.dim() == 2:
        assert torch.is_complex(x), "Must be 2-D complex-valued or 3-D real-valued with layered I/Q"
        x = torch.stack((x.real, x.imag), dim=1)
    elif x.dim() == 3:
        assert not torch.is_complex(x), "Must be 2-D complex-valued or 3-D real-valued with layered I/Q"
        assert x.shape[1] == 2, "Must be 2-D complex-valued or 3-D real-valued with layered I/Q"
    ...
    for i in range(0, x.shape[0], args.batch_size):
        y_pred[i:i+args.batch_size] = fr_module(x[i:i+args.batch_size]).detach().cpu()
